...
function setupControlPanel(noOfGroups) {
  ...
  // noofgroupsdviv = inputSec.append("div").attr("class", "noofgroupsdiv");
  // noofgroupsdviv
  //   .append("label")
  //   .attr("for", "noofgroups")
  //   .text("No of Groups (K)");
  ...
  //   }
  // });
  iterate_btn = inputSec.append("div").attr("class", "inputbox");
  iterate_btn
    .append("input")
    .attr("type", "button")
...
function initialization(K) {
  ...
  AllData.mu_estimates.push(1);
  AllData.mu_estimates.push(5);
  mu =
...
function expectation(K) {
  for (i = 0; i < K; i++) {
    ...
    AllData.clusters.jointprob[i] = AllData.clusters.likelihood[i].map(
      (x) => x * AllData.Priors[i]
    );
  }
  AllData.clusters.marginal = AllData.clusters.jointprob.reduce((row1, row2) =>
    row1.map((num, index) => num + row2[index])
  );
  for (i = 0; i < K; i++) {
